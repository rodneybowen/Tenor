...
- ~~**Stop button debounce not yet implemented**~~ — **built** Jul 3 2026 (`src/screens/VoiceScreen.tsx`). Stop disables while `sr.interim` is non-empty; 250 ms trailing debounce; dim visual. See "Detailed Flow: Voice Logging → Recording state" for the as-shipped spec.
- ~~**Waveform always animates**~~ — **built** Jul 3 2026 (`src/lib/micLevel.ts` + `src/index.css .wave--live`). Real Web-Audio amplitude, per-bar CSS vars, bars flat at silence.
...
- **Amplitude-reactive waveform (built Jul 3 2026 — `src/lib/micLevel.ts`).** The nine wave bars under the transcript are driven by real mic amplitude, not a synthetic keyframe animation. Implementation: `useMicLevel(waveRef, active)` opens a parallel `getUserMedia` stream, wires it into an `AnalyserNode` (FFT 128, smoothing 0.7), and writes per-bar amplitude 0–1 into CSS custom properties (`--w-0` … `--w-8`) on the wave container every rAF. CSS reads those vars via `calc(6px + 20px * var(--w-i, 0))`. Bytes below the silence floor (12/255) collapse to 0 → bars sit flat when nobody's talking. Voice range mapped to bins ~1–20 of the FFT so the low bars respond to vowels and the high bars to sibilants. Skipped in demo mode (no real mic to sample) and torn down cleanly on unmount (`getUserMedia` tracks stopped, AudioContext closed). Runs alongside the Web Speech API's own internal mic stream — most browsers hand out multiple streams from the same input device without complaint; if `getUserMedia` fails (permission, no device, unsupported), the hook silently no-ops and the wave stays flat.
- **Stop button — analysing gate (Jul 3 2026).** The button is disabled whenever the recognizer is holding uncommitted interim text (`sr.interim` non-empty), meaning the current utterance hasn't been finalized yet. Stopping mid-interim would drop those in-flight words from the captured transcript. A 250 ms trailing debounce keeps the button from flickering on/off between quick consecutive words — once interim clears and stays clear for 250 ms, the button re-enables. Never disabled on first render (the user may want to back out cleanly before speaking); only after at least one SR result event has fired. Visual: `opacity: 0.4`, no active-state transform, `cursor: default` while disabled. Hint copy under the button swaps to "listening for that last word…" during the disabled window.
